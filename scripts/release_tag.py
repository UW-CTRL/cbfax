...
def latest_semver_tag(prefix: str) -> tuple[Version, str] | None:
    ...
    # Tuple ordering works for semantic versions here.
    return max(candidates, key=lambda pair: (pair[0].major, pair[0].minor, pair[0].patch))
...
def main() -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Print next tag without creating it")
    parser.add_argument("--push", action="store_true", help="Push created tag to origin")
    parser.add_argument(
        "--allow-dirty",
        action="store_true",
